...
class MyTest(unittest.TestCase):
    ...
    def test_check_repository_csv_file(self):
        os.system('./runPredict.sh samplemerge')
        df = pd.read_csv(config.TEMP_CSV_PATH.replace('..', '.') + 'Repository_owhadi___sample-merge.csv',
                         header=None)
    ...
    def test_check_merge_scenario_csv_file(self):
        os.system('./runPredict.sh samplemerge')
        df = pd.read_csv(config.TEMP_CSV_PATH.replace('..', '.') + 'Merge_Scenario_owhadi___sample-merge.csv',
                         header=None)
    ...
    def test_check_merge_replay_csv_file(self):
        os.system('./runPredict.sh samplemerge')
        df = pd.read_csv(config.TEMP_CSV_PATH.replace('..', '.') + 'Merge_Replay_owhadi___sample-merge.csv',
                         header=None)
        self.assertEqual(df.values[0][1], 1)
        self.assertEqual(df.values[0][-1], 156289982)
    ...
    def test_check_merge_related_commit_csv_file(self):
        os.system('./runPredict.sh samplemerge')
        df = pd.read_csv(config.TEMP_CSV_PATH.replace('..', '.') + 'Merge_Related_Commit_owhadi___sample-merge.csv',
                         header=None)
