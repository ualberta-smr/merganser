...
class MyTest(unittest.TestCase):
    ...
    def test_check_if_paths_exist(self):
        os.system('cd ..;./runPredict.sh samplemerge')
        self.assertTrue(os.path.exists(config.REPOSITORY_PATH))
        self.assertTrue(os.path.exists(config.REPOSITORY_PATH))
        self.assertTrue(os.path.exists(config.TEMP_CSV_PATH))
        self.assertTrue(os.path.exists(config.REPOSITORY_LIST_PATH))
        self.assertTrue(os.path.exists(config.LOG_PATH))
        self.assertTrue(os.path.exists(config.QUERY_PATH))

    def test_check_repository_csv_file(self):
        os.system('cd ..;./runPredict.sh samplemerge')
        df = pd.read_csv(config.TEMP_CSV_PATH + 'Repository_owhadi___sample-merge.csv', header=None)
    ...
    def test_check_merge_scenario_csv_file(self):
        os.system('cd ..;./runPredict.sh samplemerge')
        df = pd.read_csv(config.TEMP_CSV_PATH + 'Merge_Scenario_owhadi___sample-merge.csv', header=None)
    ...
    def test_check_merge_replay_csv_file(self):
        os.system('cd ..;./runPredict.sh samplemerge')
        df = pd.read_csv(config.TEMP_CSV_PATH + 'Merge_Replay_owhadi___sample-merge.csv', header=None)
        self.assertEqual(df.values[0][1], 1)
        self.assertEqual(df.values[0][-1], 156289982)
    ...
    def test_check_merge_related_commit_csv_file(self):
        os.system('cd ..;./runPredict.sh samplemerge')
        df = pd.read_csv(config.TEMP_CSV_PATH + 'Merge_Related_Commit_owhadi___sample-merge.csv', header=None)
        print(df.values[1])
